...
export default function UpdateTransactionModal() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (parsedValue.data.transferId) return // Transfers cannot be updated

      setIsLoading(true)
      const res = await updateTransactionAction({
        data: { ...parsedValue.data, amount: parsedValue.data.amount.toString() },
      })
      setIsLoading(false)
  ...
  return (
    <Dialog handle={updateTransactionModalHandle}>
      {({ payload }) => {
        ...
        form.setFieldValue('transferId', parsedPayload.data.transferId)
        ...
        return (
          <DialogContent>
            ...
            {isTransfer && (
              <div className='rounded-md bg-amber-500/10 px-3 py-2 text-amber-600 text-sm dark:text-amber-400'>
