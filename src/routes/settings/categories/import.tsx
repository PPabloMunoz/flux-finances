...
import { importCategoriesAction } from '@/features/settings'

export const Route = createFileRoute('/settings/categories/import')({
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        const res = await importCategoriesAction({ data: { csv: content } })

        if (res.ok && res.data) {
          toast.success(`Imported ${res.data.success} categories. ${res.data.skipped} skipped.`)
          ...
          toast.error(res.error || 'Failed to import categories')
        }
    ...
    <>
      ...
      <main className='container mx-auto space-y-8 px-5 py-10'>
        <header className='mb-8 flex items-center gap-4'>
          ...
          <div>
            <h1 className='mb-1 font-medium text-2xl tracking-tight'>Import Categories</h1>
            <p className='text-muted-foreground text-sm'>Upload a CSV file to import categories</p>
          </div>
        </header>
        ...
        <div className='rounded-lg border border-border bg-card p-6'>
          <div className='mb-6'>
            ...
            <p className='text-muted-foreground text-sm'>
              Supported columns: id, user_id, name, type, color, parent_id
            </p>
            <p className='mt-1 text-muted-foreground text-xs'>
              Note: The user_id from the CSV will be replaced with your current user ID.
            </p>
          </div>
